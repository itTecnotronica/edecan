...
class FormController extends Controller
{
    ...
    {  
        ...
        if ($Solicitud <> null) {
            ...
            if ($Solicitud->tipo_de_evento->id == 1 or ($Solicitud->tipo_de_evento->id == 3 and $Solicitud->tipo_de_curso_online_id == 4)) { 
                if ($Solicitud->cant() == 1) {
            ...
            if ($Solicitud->tipo_de_evento->id == 3 and $Solicitud->fecha_de_inicio_del_curso_online <> '') {
                ...
                $fecha_inicio_format = $gCon->FormatoFecha($Solicitud->fecha_de_inicio_del_curso_online);
            ...
            if ($Solicitud->hash == $hash or ($Solicitud->hash ==  '' and $hash = 'nulo')) {    
                ...
                if ($Solicitud->institucion_id == 2) {
                    ...
                    $imagen_top = env('PATH_PUBLIC').'img/sol-de-acuario-chico-isologo.png';
    ...
    public function RegistrarInscripcion(Request $request) {
        ...
        if ($Solicitud->tipo_de_evento_id == 4 and $Solicitud->pais_id <> '') {
        ...
        if (($Solicitud->tipo_de_evento_id == 3 and $Solicitud->tipo_de_curso_online_id <> 4) or $Solicitud->tipo_de_evento_id == 4) {
            ...
            if ($Solicitud->tipo_de_evento_id == 3 or ($Solicitud->tipo_de_evento_id == 4 and $Solicitud->pais_id == '')) {
        ...
        $Inscripcion = Inscripcion::where('solicitud_id', $solicitud_id)->first();
        $sesion_id = Session()->get('sesion_id');
    ...
    public function traerInscripciones($solicitud_id, $campania_id = 0, $offset = 0, $cant_x_pagina = null, $grupo = null, $criterio = null, $historico = false)
    {
        ...
        if ($historico) {
            ...
            if ($localidad_id_h <> '') {
                ...
                $whereRaw .= " and (s.localidad_id IS NOT NULL and (s.localidad_id = $localidad_id_h or (inscripciones.ciudad like '%$localidad_h%' and s.pais_id = inscripciones.pais_id)))";
            }
            else {
            ...
            //$whereRaw .= " and inscripciones.id = 225993";
        ...
        if ($criterio <> null) {
            $whereRaw .= " and (";
            $whereRaw .= "inscripciones.id = '".$criterio."' or ";
            $whereRaw .= "inscripciones.solicitud_original = '".$criterio."' or ";
            $whereRaw .= "inscripciones.nombre like '%".$criterio."%' or ";
            $whereRaw .= "inscripciones.apellido like '%".$criterio."%' or ";
            $whereRaw .= "inscripciones.ciudad like '%".$criterio."%' or ";
            $whereRaw .= "p.pais like '%".$criterio."%' or ";
            $whereRaw .= "inscripciones.codigo_alumno like '%".$criterio."%' or ";
            $whereRaw .= "inscripciones.celular like '%".$criterio."%' or ";
            $whereRaw .= "inscripciones.email_correo like '%".$criterio."%')";
        }
        ...
        ->leftjoin('envios as en', function ($join) {
            $join->on('s.id', '=', 'en.solicitud_id')->on('en.inscripcion_id', '=', 'inscripciones.id');
        })
    ...
    public function listInscriptosPaginar($solicitud_id, $hash, $pagina, $historico)
    {  
        $cant_x_pagina = $this->cant_x_pagina;
        $cant_total_inscriptos = Inscripcion::whereRaw("(solicitud_id = $solicitud_id or (solicitud_original = $solicitud_id and causa_de_cambio_de_solicitud_id in (1,4)))")->count();
        ...
        if ($Solicitud->hash == $hash) {
            ...
            if (!$historico) {
                ...
                $Modelos_alumno = null;
                $Causas_de_baja = null;
                $Causas_de_cambio_de_solicitud = null;
                ...
                foreach ($Inscripciones as $index => $Inscripcion) {
                    ...
                    if ($Solicitud->tipo_de_evento_id == 3) {
                        $linea = [
                            ...
                            $Inscripcion->nombre, 
                            $this->telefono($Inscripcion->celular), 
                            $Inscripcion->email_correo, 
                            $Inscripcion->ciudad,  
                            $pais, 
                            $Inscripcion->sino_envio_pedido_de_confirmacion, 
                            $Inscripcion->sino_cancelo, 
                            $Inscripcion->grupo, 
                            $Inscripcion->codigo_alumno
                            ];

                        foreach ($lecciones as $leccion) {
                            $asistencia = $Inscripcion->asistencias->where('leccion_id', $leccion->id)->all();
    ...
    */
            ...
            $hash = md5(ENV('PREFIJO_HASH').$inscripcion_id);
            $url_qrcode = ENV('PATH_PUBLIC').'f/registrar-asistencia/'.$inscripcion_id.'/'.$hash;
