...
class HomeController extends Controller
{
    ...
    public function registros_home() {
        ...
        if (Auth::user()->rol_de_usuario_id <= 2) {
            ...
            ->get();
            $Autorizaciones = User::
            select(DB::Raw('Users.*, p.pais pais_desc, e.equipo equipo_desc'))
            ->leftjoin('equipos as e', 'e.id', '=', 'users.equipo_id')
        ...
        if (Auth::user()->rol_de_usuario_id == 2) {
            ...
            ->leftjoin('equipos as e', 'e.id', '=', 'users.equipo_id')
            ->leftjoin('paises as p', 'p.id', '=', 'users.pais_id')
            ->whereRaw("rol_de_usuario_id IS NULL and ($equipo_where equipo_id in (SELECT ue.equipo_id from usuarios_por_equipo as ue WHERE ue.user_id = $user_id))")
            ->get();
    ...
    {
        $tipos_de_evento = Tipo_de_evento::where('id', '<>', 3)->get();
