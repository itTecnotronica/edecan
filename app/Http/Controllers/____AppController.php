...
class AppController extends Controller
{
   ...
   public function getDebitos($pais_id,  $token) {
    ...
    public function saveDebito($id, $tb_tarjeta_id, $tb_tipo_de_tarjeta_id,  $tb_persona_id, $numero_de_tarjeta,$monto, $observaciones, $fechaVto, $token )
    {
            if ($token == 'gapp') { 
 ...
 public function deleteDebito($id, $token )
    ...
    public function getCarnet($pais_id, $persona_id, $carnet_id, $token) {

        if ($token == 'gapp') {
            $whereRaw = "1=1";
            if ($persona_id != '0')
            {
                $whereRaw =  $whereRaw . " and (car.tb_persona_id=$persona_id)";  
            }
            if ($carnet_id != '0')
            {
                $whereRaw =  $whereRaw . " and (car.id=$carnet_id)";  
            }
    ...
    public function getCiudades($idioma_id, $pais_id) {
        ...
        $Localidades = DB::table('paises as p')
            ...
            ->whereRaw('((s.tipo_de_evento_id in (1, 2) AND DATEDIFF(NOW(), fe.fecha_de_inicio)  <= 15) or (s.tipo_de_evento_id = 3 AND DATEDIFF(NOW(), s.fecha_de_inicio_del_curso_online)  <= 15) or DATEDIFF(NOW(), s.created_at)  <= 15)')
