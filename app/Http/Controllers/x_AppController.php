...
class AppController extends Controller
{
 ...
 public function deleteDebito($id, $token )
        ...
        if ($token == 'gapp') {
           ...
           else
           {
            ...
            if ($opcion=='created_at'){
                $whereRaw =  " (car.created_at is not null and fecha_visto is null and fecha_autorizado is null and fecha_de_pago is null and fecha_de_confeccion is null and fecha_envio is null) " ;
            }
            if ($opcion=='fecha_visto'){
                $whereRaw =  " (fecha_visto is not null and fecha_autorizado is null and fecha_de_pago is null and fecha_de_confeccion is null and fecha_envio is null) " ;
            }
            if ($opcion=='fecha_autorizado'){
                $whereRaw =  " (fecha_autorizado is not null and fecha_de_pago is null and fecha_de_confeccion is null and fecha_envio is null) " ;
            }
            if ($opcion=='fecha_de_pago'){
                $whereRaw =  " (fecha_de_pago is not null and fecha_de_confeccion is null and fecha_envio is null) " ;
            }
            if ($opcion=='fecha_de_confeccion'){
                $whereRaw =  " (fecha_de_confeccion is not null and fecha_envio is null) " ;
            }
            if ($opcion=='fecha_envio'){
                $whereRaw =  " (fecha_envio is not null) " ;
            }
    ...
    public function getMaterialesSearch($idioma_id, $token, $value, $publico) {
        ...
        if ($token == 'gapp') {
            $whereRaw = " (mat.titulo like '%$value%' OR mat.descripcion like '%$value%') "; 
            if ($publico == 1){
                $whereRaw = $whereRaw . " and (mat.sino_es_un_material_publico = 'SI')";  
            }
    ...
    public function getMiembrosObservaciones( $token) {
        ...
        try {  
            ...
            if ($token == 'gapp') {           
                        ...
                        ->select(DB::Raw('  mi.`name` nombre,
                                            mi.registration,
                                            mi.sino_isMissionActive ')) 
                        ->leftjoin('app_miembros as mi', 'mi.registration', '=', 'mie.miembro_id')
                        ->groupBy('mi.name')
                        ->groupBy('mi.registration')
                        ->groupBy('mi.sino_isMissionActive')
                        ->get(); 
    ...
    public function getMiembroAporte( $idAporte, $lumisial, $year, $token) {
    ...
    public function getAportesPorLumisial( $token) {
   ...
   public function getMovimientosContables(  $responsable , $token) {

    try {  
        $whereRaw = "1=1"; 
        if ($responsable != '-')
        {
            $whereRaw = "  (mc.responsable='$responsable')";
        } 
        if ($token == 'gapp') {           
