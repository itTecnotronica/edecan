...
class Inscripcion extends Model
{
    ...
    public function planilla_original()
    {
        ...
        if ($Origen->count() > 0) {
            ...
            $mensaje = '<strong>Solicitud Original</strong>: '.$Solicitud->id.' | Tutor: '.$Solicitud->nombre_responsable_de_inscripciones.' cel: '.$Solicitud->celular_responsable_de_inscripciones;
            $boton_wa = '<a href="https://api.whatsapp.com/send?phone='.$Solicitud->celular_wa($Solicitud->celular_responsable_de_inscripciones).'" target="_blank">';
    ...
    {
        $a_frase = explode(' ', $frase);
        $fraseCapital = '';
        foreach ($a_frase as $nombre) {
            $fraseCapital .= $nombre.' ';
        }
    ...
    public function url_certificado()
    {
        $inscripto_id = $this->id;
        $hash = md5(ENV('PREFIJO_HASH').$inscripto_id);
