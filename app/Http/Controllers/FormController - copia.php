...
class FormController extends Controller
{
    ...
    public function RegistrarInscripcion(Request $request) {
        ...
        */
        ...
        if (isset($_POST['codigo_pais'])) {
            ...
            $codigo_pais = ($codigo_pais == '+54' and substr($celular,0,1) <> 9) ? $codigo_pais.'9' : $codigo_pais;
            $celular = $codigo_pais.$celular;
        }
        */
    ...
    public function traerInscripciones($solicitud_id, $campania_id = 0, $offset = 0, $cant_x_pagina = null, $grupo = null, $criterio = null, $historico = false, $recupero = array())
    {
        ...
        if ($historico) {
            ...
            ->whereRaw($whereRaw)
            ->groupBy(DB::Raw('inscripciones.apellido, inscripciones.nombre, inscripciones.celular, inscripciones.email_correo'))
            ->get()
            ->count();
        ...
        $Inscripciones = Inscripcion::select(DB::Raw($selectRaw))
        ->whereRaw($whereRaw)
        ->leftjoin('fechas_de_evento as f', 'f.id', '=', 'inscripciones.fecha_de_evento_id')
        ->join('solicitudes as s', 's.id', '=', 'inscripciones.solicitud_id')
