    ...
    public function getEventos2($idioma_id, $pais_id, $localidad_id) {
        ...
        if ($localidad_id <> -1) {
            ...
            $Eventos = DB::table('localidades as l')
                ...
                //->whereRaw('((s.tipo_de_evento_id in (1, 2) AND fe.fecha_de_inicio >= "2023-02-20") or (s.tipo_de_evento_id = 3 AND s.fecha_de_inicio_del_curso_online >= "2023-02-20") or s.created_at >= "2023-02-20")')
                ->whereRaw('((s.tipo_de_evento_id in (1, 2) AND DATEDIFF(NOW(), fe.fecha_de_inicio)  <= 15) or (s.tipo_de_evento_id = 3 AND DATEDIFF(NOW(), s.fecha_de_inicio_del_curso_online)  <= 15) or DATEDIFF(NOW(), s.created_at)  <= 15)')
                ->whereRaw("(s.sino_es_campania_de_capacitacion IS NULL OR s.sino_es_campania_de_capacitacion = '' OR s.sino_es_campania_de_capacitacion = 'NO')")
                ->whereRaw("(s.sino_aprobado_finalizada IS NULL OR s.sino_aprobado_finalizada = '' OR s.sino_aprobado_finalizada = 'NO')")
                ->get();
